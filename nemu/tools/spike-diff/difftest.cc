...
void sim_t::diff_get_regs(void* diff_context) {
  ...
  // ctx->mcause = state->csrmap[0x342]->read();//mcause
  // ctx->mepc = state->csrmap[0x341]->read();//mepc
  // ctx->mstatus = state->csrmap[0x300]->read();//mstatus
  // ctx->mtvec = state->csrmap[0x305]->read();//mtvec
}
...
void sim_t::diff_set_regs(void* diff_context) {
  ...
  //  state->csrmap[0x342]->write(ctx->mcause); 
  //  state->csrmap[0x341]->write(ctx->mepc); 
  //  state->csrmap[0x300]->write(ctx->mstatus); 
  //  state->csrmap[0x305]->write(ctx->mtvec); 
}
